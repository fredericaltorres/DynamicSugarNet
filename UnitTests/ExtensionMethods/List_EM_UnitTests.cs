...
namespace DynamicSugarSharp_UnitTests
{
    ...
    public class List_EM_UnitTests
    {
        ...
        public void ToFile_FromFile()
        {
            ...
            var numberFrom1To10 = DS.Range(10);
            var fileName = Path.Combine(Environment.GetEnvironmentVariable("TEMP"), @"DSSharpLibrary_UnitTests.txt");
            DeleteFile(fileName);

            numberFrom1To10.ToFile(fileName, true);
            Assert.IsTrue(File.Exists(fileName));

            var expected = new List<string>();
            foreach (var i in numberFrom1To10) expected.Add(i.ToString());

            var l10 = DS.List<string>().FromFile(fileName);
            DS.Assert.AreEqual(expected, l10);

            var l11 = DS.List<string>("").FromFile(fileName);
            var expected2 = new List<string>();
            expected2.Add("");
            expected2.AddRange(expected);
            DS.Assert.AreEqual(expected2, l11);
            DS.Assert.AreNotEqual(expected, l11);

            var l1 = DS.ListHelper.FromFile<string>(fileName);
            DS.Assert.AreEqual(expected, l1);

            var l2 = DS.ListHelper.FromFile<int>(fileName);
            DS.Assert.AreEqual(numberFrom1To10, l2);
        }
